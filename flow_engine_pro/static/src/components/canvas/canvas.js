...
export class FlowCanvas extends Component {
  ...
  _resolveEffectiveEndpoint(nodeId, cameFromEdgeId, visited = new Set()) {
    ...
    visited.add(nodeId);
    for (const e of this.props.canvasData.edges) {
      if (e.id === cameFromEdgeId) continue;
      const sId = e.sourceNode || e.source;
      const tId = e.targetNode || e.target;
      if (sId === nodeId) {
        const result = this._resolveEffectiveEndpoint(tId, e.id, visited);
        if (result) return result;
      } else if (tId === nodeId) {
        const result = this._resolveEffectiveEndpoint(sId, e.id, visited);
        if (result) return result;
      }
    }
    return null;
  ...
  _isFlowLine(edge) {
    ...
    const end1 = this._resolveEffectiveEndpoint(n1id, edge.id);
    const end2 = this._resolveEffectiveEndpoint(n2id, edge.id);
    return FlowCanvas._isFlowShapeNode(end1) && FlowCanvas._isFlowShapeNode(end2);
  }
  ...
  _computeEdgeSideInfo() {
    ...
    for (const edge of this.props.canvasData.edges) {
      ...
      if (!this._isFlowLine(edge)) continue; // Comment lines never use stub routing
  ...
  _getStubbedPointsFor(edge, n1, n2, edgeSideInfo) {
    ...
    return MathUtils.buildStubbedOrthogonalPoints(n1, sideInfo.side1, n2, sideInfo.side2, null, sideInfo.exitOffset, sideInfo.entryOffset, this.props.canvasData.nodes);
  }
  ...
  // point at all (see _computeEdgeSideInfo's exitOffset/entryOffset
  ...
  _getJunctionPoints(edgeSideInfo) {
    ...
    for (const { node, side, count, offset } of [...exitGroups.values(), ...entryGroups.values()]) {
      ...
      // with the opposite direction (see _computeEdgeSideInfo) got
  ...
  _findHitNode(worldPos) {
    if (!this.props.canvasData || !this.props.canvasData.nodes) return null;

    // Search backwards (top-to-bottom in z-index)
    for (let i = this.props.canvasData.nodes.length - 1; i >= 0; i--) {
      const node = this.props.canvasData.nodes[i];
    ...
    for (const edge of this.props.canvasData.edges) {
      ...
      if (isSolid) {
        ...
        const segments = points
          ...
          : this._getOrthogonalSegments(n1, n2);
        for (const seg of segments) {
          if (this._pointToSegmentDist(worldPos.x, worldPos.y, seg.p1.x, seg.p1.y, seg.p2.x, seg.p2.y) < THRESHOLD) return edge;
        }
      } else {
        if (this._pointToSegmentDist(worldPos.x, worldPos.y, n1.x, n1.y, n2.x, n2.y) < THRESHOLD) return edge;
      }
    }
    return null;
  }
  ...
  _autoLayout(includeComments = false) {
    ...
    const edges = this.props.canvasData.edges || [];
