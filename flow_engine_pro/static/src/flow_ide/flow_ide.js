...
export class FlowIDE extends Component {
  ...
  openReviewPage() {
    window.open('https://apps.odoo.com/apps/modules/17.0/flow_engine_pro#comment', '_blank', 'noopener,noreferrer');
  ...
  async _computeSidebarSearchCounts() {
    ...
    // Odoo 17's orm.webReadGroup signature is (model, domain, fields,
  ...
  async loadDiagram(diagramId) {
    ...
    if (result && result.length) {
      ...
      // Odoo 17 note: the unanchored /id=\d+/ this used to use matched
      // the TAIL of 'menu_id=' or 'cids=' (both legitimately contain the
      // substring 'id=') whenever no diagram had been opened yet in this
      // hash, silently corrupting menu_id into the diagram's own id and
      // hijacking the whole app to an unrelated menu. Odoo 19/18's
      // pretty-URL routing (/odoo/...) doesn't populate the hash this
      // way so the bug never showed there, but it's a real latent bug
      // on any hash-based route - found while porting to Odoo 17, where
      // the classic #action=..&cids=..&menu_id=.. hash format exposed
      // it immediately. Now only matches a standalone id= param
      // (preceded by & or #).
